...
void frd(double *co,ITG *nk,ITG *kon,ITG *ipkon,char *lakon,ITG *ne0,
  ...
  /* UB21: 6-component vector for BFOR beam forces/moments */
  ITG ncompvector6=6,ifieldvector6[6]={1,1,1,1,1,1},icompvector6[6]={0,1,2,3,4,5},
    nfieldvector6[2]={1,0};
  ...
  /* UB21: count UB21 elements and compute virtual node offset.
     For each UB21 element we add 9 interior station nodes (stations 1..9,
     i.e. 10%..90% along the beam). Station 0 and 10 coincide with the real
     element end-nodes. The virtual nodes get IDs starting at virt_node_offset. */
  for(i=0; i<*nk; i++) if(i+1 > max_node_id) max_node_id = i+1;
  virt_node_offset = max_node_id + 1;
  n_ub21_elems = 0;
  for(i=0; i<*ne0; i++){
    if(ipkon[i] >= 0 &&
       (strcmp1(&lakon[8*i],"U1")==0 || strcmp1(&lakon[8*i],"UB21")==0)){
      n_ub21_elems++;
    }
  }
  nout     += n_ub21_elems * 9;
  noutplus += n_ub21_elems * 9;
  ...
  if((*kode==1)&&((*nmethod!=5)||(*mode!=0))){
    ...
    /* UB21: write 9 interior virtual station node coordinates per UB21 element.
       Virtual nodes interpolate linearly between end-nodes at xi=0.1..0.9. */
    current_virt_id = virt_node_offset;
    for(i=0; i<*ne0; i++){
      if(ipkon[i] >= 0 &&
         (strcmp1(&lakon[8*i],"U1")==0 || strcmp1(&lakon[8*i],"UB21")==0)){
        ITG indexe_v = ipkon[i];
        n1 = kon[indexe_v] - 1;
        n2 = kon[indexe_v+1] - 1;
        for(k=1; k<=9; k++){
          double xi  = (double)k / 10.0;
          double vx  = co[3*n1]   + xi*(co[3*n2]   - co[3*n1]);
          double vy  = co[3*n1+1] + xi*(co[3*n2+1] - co[3*n1+1]);
          double vz  = co[3*n1+2] + xi*(co[3*n2+2] - co[3*n1+2]);
          if(strcmp1(output,"asc")==0){
            fprintf(f1,"%3s%10" ITGFORMAT "%12.5E%12.5E%12.5E\n",
                    m1,current_virt_id,(float)vx,(float)vy,(float)vz);
          }else{
            iw=(int)current_virt_id; fwrite(&iw,sizeof(int),1,f1);
            fwrite(&vx,sizeof(double),1,f1);
            fwrite(&vy,sizeof(double),1,f1);
            fwrite(&vz,sizeof(double),1,f1);
          }
          current_virt_id++;
        }
      }
	...
	if(((ipkon[i]<=-1)&&(ioutall==0))||(ipkon[i]==-1)){
	  ...
	  /* user element: UB21 expands to 10 line sub-elements; all others skipped */

	}else if(strcmp1(&lakon[8*i],"U")==0){
	  if(strcmp1(&lakon[8*i],"U1")==0 || strcmp1(&lakon[8*i],"UB21")==0){
	    nelout += 10;
	  }
	  continue;
	}
	...
	continue;
      }else if(strcmp1(&lakon[8*i],"U")==0 && (strcmp1(&lakon[8*i],"U1")!=0 && strcmp1(&lakon[8*i],"UB21")!=0)){
	continue;
      }else{
	indexe=ipkon[i];
      }
	...
	if(strcmp1(output,"asc")==0){
      ...
      /* UB21: write 10 CGX line sub-elements connecting:
         sub 0: real_n1 -> virt_1
         sub 1: virt_1  -> virt_2  ...  sub 8: virt_8 -> virt_9
         sub 9: virt_9  -> real_n2
         Virtual node IDs are packed sequentially from virt_node_offset. */
      }else if(strcmp1(&lakon[8*i],"U1")==0 || strcmp1(&lakon[8*i],"UB21")==0){
	ITG sub_base = (i+1)*100; /* unique sub-element IDs per parent element */
	ITG rn1 = kon[indexe];         /* 1-based real end-node 1 */
	ITG rn2 = kon[indexe+1];       /* 1-based real end-node 2 */
	for(k=0; k<10; k++){
	  ITG sn1, sn2;
	  if(k==0){
	    sn1 = rn1;
	  ...
	  if(strcmp1(output,"asc")==0){
	    ...
	    iw=(int)(sub_base+k); fwrite(&iw,sizeof(int),1,f1);
	    iw=(int)ip11;          fwrite(&iw,sizeof(int),1,f1);
	    iw=(int)ip0;           fwrite(&iw,sizeof(int),1,f1);
	    iw=(int)imat;          fwrite(&iw,sizeof(int),1,f1);
	    iw=(int)sn1;           fwrite(&iw,sizeof(int),1,f1);
  ...
  if((*nmethod!=5)||(*mode==-1)){
	...
	frdvector(v,&iset,ntrans,filab,&nkcoords,inum,m1,inotr,
		  trab,co,istartset,iendset,ialset,mi,ngraph,f1,output,NULL,
		  &ioutall);

	/* UB21: append interpolated displacement values for 9 virtual station nodes */
	current_virt_id = virt_node_offset;
	for(i=0; i<*ne0; i++){
	  if(ipkon[i] >= 0 &&
	     (strcmp1(&lakon[8*i],"U1")==0 || strcmp1(&lakon[8*i],"UB21")==0)){
	    ITG indexe_d = ipkon[i];
	    n1 = kon[indexe_d] - 1;  /* 0-based */
	    n2 = kon[indexe_d+1] - 1;
	    for(k=1; k<=9; k++){
	      double xi = (double)k / 10.0;
	      float u1 = (float)(v[mt*n1+1] + xi*(v[mt*n2+1] - v[mt*n1+1]));
	      float u2 = (float)(v[mt*n1+2] + xi*(v[mt*n2+2] - v[mt*n1+2]));
	      float u3 = (float)(v[mt*n1+3] + xi*(v[mt*n2+3] - v[mt*n1+3]));
	      float uall = sqrtf(u1*u1 + u2*u2 + u3*u3);
	      if(strcmp1(output,"asc")==0){
		fprintf(f1,"%3s%10" ITGFORMAT "%12.5E%12.5E%12.5E\n",
			m1,current_virt_id,u1,u2,u3);
		fprintf(f1,"%3s          %12.5E\n",m2,uall);
	      }else{
		iw=(int)current_virt_id; fwrite(&iw,sizeof(int),1,f1);
		fwrite(&u1,sizeof(float),1,f1); fwrite(&u2,sizeof(float),1,f1);
		fwrite(&u3,sizeof(float),1,f1);
		fwrite(&uall,sizeof(float),1,f1);
	      }
	      current_virt_id++;
	    }
	  }
	...
	for(i=0; i<*ne0; i++){
	  if(ipkon[i] >= 0 &&
	    ...
	    for(k=1; k<=9; k++){
	      double xi = (double)k / 10.0;
	      /* v layout: v[(mi[1]+1)*node + dof], dof 1..mi[1] */
	      float u1 = (float)(v[mt*n1+1] + xi*(v[mt*n2+1] - v[mt*n1+1]));
	      float u2 = (float)(v[mt*n1+2] + xi*(v[mt*n2+2] - v[mt*n1+2]));
	      float u3 = (float)(v[mt*n1+3] + xi*(v[mt*n2+3] - v[mt*n1+3]));
	      float u4 = (float)(v[mt*n1+4] + xi*(v[mt*n2+4] - v[mt*n1+4]));
	      float u5 = (float)(v[mt*n1+5] + xi*(v[mt*n2+5] - v[mt*n1+5]));
	      float u6 = (float)(v[mt*n1+6] + xi*(v[mt*n2+6] - v[mt*n1+6]));
	      float uall = sqrtf(u1*u1 + u2*u2 + u3*u3);
	      if(strcmp1(output,"asc")==0){
		fprintf(f1,"%3s%10" ITGFORMAT "%12.5E%12.5E%12.5E%12.5E%12.5E%12.5E\n",
			m1,current_virt_id,u1,u2,u3,u4,u5,u6);
		fprintf(f1,"%3s          %12.5E\n",m2,uall);
	      }else{
		iw=(int)current_virt_id; fwrite(&iw,sizeof(int),1,f1);
		fwrite(&u1,sizeof(float),1,f1); fwrite(&u2,sizeof(float),1,f1);
		fwrite(&u3,sizeof(float),1,f1); fwrite(&u4,sizeof(float),1,f1);
		fwrite(&u5,sizeof(float),1,f1); fwrite(&u6,sizeof(float),1,f1);
		fwrite(&uall,sizeof(float),1,f1);
	      }
	      current_virt_id++;
	    }
  ...
  if((*nmethod!=5)||(*mode==-1)){
    if((strcmp1(&filab[174],"S   ")==0)&&(*ithermal!=2)){
      ...
      /* UB21 STRESS: overwrite physical end-node stn values from ub21_stx
	 (stations 0 and 10), then append 9 virtual station nodes */
      for(i=0; i<*ne0; i++){
	if(ipkon[i] >= 0 &&
	   (strcmp1(&lakon[8*i],"U1")==0 || strcmp1(&lakon[8*i],"UB21")==0)){
	  double ub21buf[6*11];
	  ITG nelem_1 = i + 1;
	  FORTRAN(getub21stx,(&nelem_1, ub21buf));
	  ITG indexe_s = ipkon[i];
	  n1 = kon[indexe_s] - 1;
	  n2 = kon[indexe_s+1] - 1;
	  for(j=0; j<6; j++){
	    stn[6*n1+j] = ub21buf[6*0 + j];   /* station 0 -> real node 1 */
	    stn[6*n2+j] = ub21buf[6*10 + j];  /* station 10 -> real node 2 */
	  }
	  /* Populate SYY and SZZ with SXX for 3D tensor display compatibility in CGX */
	  stn[6*n1+1] = stn[6*n1+0];
	  stn[6*n1+2] = stn[6*n1+0];
	  stn[6*n2+1] = stn[6*n2+0];
	  stn[6*n2+2] = stn[6*n2+0];
	}
      }
      /* re-issue frdselect with corrected stn to update physical nodes */
      frdselect(stn,stn,&iset,&nkcoords,inum,m1,istartset,iendset,
		ialset,ngraph,&ncomptensor,ifieldtensor,icomptensor,
		nfieldtensor,&iselect,m2,f1,output,NULL);
      /* write virtual station nodes (stations 1..9) */
      current_virt_id = virt_node_offset;
      for(i=0; i<*ne0; i++){
	if(ipkon[i] >= 0 &&
	   (strcmp1(&lakon[8*i],"U1")==0 || strcmp1(&lakon[8*i],"UB21")==0)){
	  ...
	  FORTRAN(getub21stx,(&nelem_1, ub21buf));
	  for(k=1; k<=9; k++){
	    /* ub21buf[6*station + component], component order: sigma,tau_xy,tau_xz,tau_tor,My,Mz */
	    float sxx = (float)ub21buf[6*k+0];
	    float syy = sxx; /* 1D beam: assign axial/bending stress to 3D tensor components for multi-face rendering in CGX */
	    float szz = sxx;
	    float sxy = (float)ub21buf[6*k+3];
	    float syz = (float)ub21buf[6*k+5];
	    float szx = (float)ub21buf[6*k+4];
	    if(strcmp1(output,"asc")==0){
	      fprintf(f1,"%3s%10" ITGFORMAT "%12.5E%12.5E%12.5E%12.5E%12.5E%12.5E\n",
		      m1,current_virt_id,sxx,syy,szz,sxy,syz,szx);
	    }else{
	      iw=(int)current_virt_id; fwrite(&iw,sizeof(int),1,f1);
	      fwrite(&sxx,sizeof(float),1,f1); fwrite(&syy,sizeof(float),1,f1);
	      fwrite(&szz,sizeof(float),1,f1); fwrite(&sxy,sizeof(float),1,f1);
	      fwrite(&syz,sizeof(float),1,f1); fwrite(&szx,sizeof(float),1,f1);
	    }
	    current_virt_id++;
	  }
	}
  ...
  /* UB21 BFOR: beam internal forces/moments — same 11-station layout as STRESS.
     This dataset is unique to UB21 and does not exist in standard CCX.
     Field order: NX (axial), VY, VZ (shear), TX (torsion), MY, MZ (moments). */
  if((*nmethod!=5)||(*mode==-1)){
    if((strcmp1(&filab[174],"S   ")==0)&&(*ithermal!=2)){
      iselect=1;
      frdset(&filab[174],set,&iset,istartset,iendset,ialset,
      ...
      frdheader(&icounter,&oner,time,&pi,noddiam,cs,&null,mode,
		&noutloc,description,kode,nmethod,f1,output,istep,iinc);
      fprintf(f1," -4  BFOR        6    1\n");
      fprintf(f1," -5  NX          1    1    0    0\n");
      fprintf(f1," -5  VY          1    1    0    0\n");
      fprintf(f1," -5  VZ          1    1    0    0\n");
      fprintf(f1," -5  TX          1    1    0    0\n");
      fprintf(f1," -5  MY          1    1    0    0\n");
      fprintf(f1," -5  MZ          1    1    0    0\n");
      /* fill stn end-nodes from ub21_stx stations 0 and 10 */
      for(i=0; i<*ne0; i++){
	if(ipkon[i] >= 0 &&
	   (strcmp1(&lakon[8*i],"U1")==0 || strcmp1(&lakon[8*i],"UB21")==0)){
	  double ub21buf[6*11];
	  ITG nelem_1 = i + 1;
	  FORTRAN(getub21stx,(&nelem_1, ub21buf));
	  ITG indexe_b = ipkon[i];
	  n1 = kon[indexe_b] - 1;
	  n2 = kon[indexe_b+1] - 1;
	  for(j=0; j<6; j++){
	    stn[6*n1+j] = ub21buf[6*0 + j];
	    stn[6*n2+j] = ub21buf[6*10 + j];
	  }
	}
      }
      frdselect(stn,stn,&iset,&nkcoords,inum,m1,istartset,iendset,
		ialset,ngraph,&ncompvector6,ifieldvector6,icompvector6,
		nfieldvector6,&iselect,m2,f1,output,NULL);
      /* write virtual station nodes (stations 1..9) */
      current_virt_id = virt_node_offset;
      for(i=0; i<*ne0; i++){
	if(ipkon[i] >= 0 &&
	  ...
	  FORTRAN(getub21stx,(&nelem_1, ub21buf));
	  for(k=1; k<=9; k++){
	    float f1_v=(float)ub21buf[6*k+0], f2_v=(float)ub21buf[6*k+1];
	    float f3_v=(float)ub21buf[6*k+2], f4_v=(float)ub21buf[6*k+3];
	    float f5_v=(float)ub21buf[6*k+4], f6_v=(float)ub21buf[6*k+5];
	    if(strcmp1(output,"asc")==0){
	      fprintf(f1,"%3s%10" ITGFORMAT "%12.5E%12.5E%12.5E%12.5E%12.5E%12.5E\n",
		      m1,current_virt_id,f1_v,f2_v,f3_v,f4_v,f5_v,f6_v);
	    }else{
	      iw=(int)current_virt_id; fwrite(&iw,sizeof(int),1,f1);
	      fwrite(&f1_v,sizeof(float),1,f1); fwrite(&f2_v,sizeof(float),1,f1);
	      fwrite(&f3_v,sizeof(float),1,f1); fwrite(&f4_v,sizeof(float),1,f1);
	      fwrite(&f5_v,sizeof(float),1,f1); fwrite(&f6_v,sizeof(float),1,f1);
	    }
	    current_virt_id++;
	  }
	}
