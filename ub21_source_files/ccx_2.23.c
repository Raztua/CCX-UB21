...
int main(int argc,char *argv[])
{
  ...
  printf("\n************************************************************\n\n");
#ifdef INTSIZE64
  printf("CalculiX Version 2.23 i8, Copyright(C) 1998-2025 Guido Dhondt\n");
#else
  printf("CalculiX Version 2.23, Copyright(C) 1998-2025 Guido Dhondt\n");
#endif
  ...
  while(istat>=0){
	...
	NNEW(prop,double,nprop_);
      ...
      /* Added ||(ne_!=0) and +36*ne_ / +ne_ / +24*ne_ to allocate offsets/normals for custom user elements (e.g. UB21, UB32) */
      if((ne1d!=0)||(ne2d!=0)||(ne_!=0)){
	NNEW(iponor,ITG,2*nkon_);
	for(i=0;i<2*nkon_;i++) iponor[i]=-1;
	NNEW(xnor,double,36*ne1d+24*ne2d+36*ne_);
	NNEW(knor,ITG,24*(ne1d+ne2d+ne_)*(mi[2]+1));
	NNEW(thickn,double,2*nk_);
	NNEW(thicke,double,mi[2]*nkon_);
	NNEW(offset,double,2*ne_);
	NNEW(iponoel,ITG,nk_);
	NNEW(inoel,ITG,9*ne1d+24*ne2d+24*ne_);
	NNEW(rig,ITG,nk_);
	...
	RENEW(prop,double,nprop);
        ...
        /* Keep ielprop allocated and set to -1 when nprop_ == 0 to avoid NULL pointer crash when user elements lookup ielprop */
	RENEW(ielprop,ITG,ne);
	for(i=0;i<ne;i++) ielprop[i]=-1;
	SFREE(prop);
      }
	...
	SFREE(coeffc);SFREE(ikdc);SFREE(edc);
      ...
      /* Added ||(ne!=0) to allocate offsets/normals for custom user elements and infree[2]>0 check to avoid negative realloc */
      if((ne1d!=0)||(ne2d!=0)||(ne!=0)){
	...
	for(i=mi[2]*ne;i<mi[2]*(ne+nload);i++)ielorien[i]=0;
      ...
      /* Keep ielprop allocated if any elements exist to prevent NULL pointer crash, and add ||(ne!=0) to allocate offset for user elements */
      if(nprop>0 || ne>0) RENEW(ielprop,ITG,ne);
      if((ne1d!=0)||(ne2d!=0)||(ne!=0)) RENEW(offset,double,2*ne);
      RENEW(nelemload,ITG,2*nload);
      RENEW(sideload,char,20*nload);
      RENEW(xload,double,2*nload);
      RENEW(xloadold,double,2*nload);
    ...
    if(network>0){
      ...
      /* Keep ielprop allocated if any elements exist to prevent NULL pointer crash, and add ||(ne1!=0) to allocate offset for user elements */
      if(nprop>0 || ne1>0) RENEW(ielprop,ITG,ne1);
      if((ne1d!=0)||(ne2d!=0)||(ne1!=0)) RENEW(offset,double,2*ne1);
